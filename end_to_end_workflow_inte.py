...
def fetch_1_min_data_CE(token_CE, st_date_1min, end_date_1min, high, low):
    print(
        f"Params - token_CE: {token_CE}, st_date: {st_date_1min}, end_date: {end_date_1min}, high: {high}, low: {low}, current_minute: {datetime.now().minute}")
    candle_df_ce_1 = hist_data(token_CE, "ONE_MINUTE", st_date_1min, end_date_1min, instrument_list)
    close_price = candle_df_ce_1["close"].iloc[-1]
    if close_price > high:
        print("High breached")
        place_order(token_CE, high, low, st_date_1min, end_date_1min)
    print(f"Close price of 1-minute candle for CE: {close_price}")

def fetch_1_min_data_PE(token_PE, st_date_1min, end_date_1min):
    candle_df_pe_1 = hist_data(token_PE, "ONE_MINUTE", st_date_1min, end_date_1min, instrument_list)
    close_price = candle_df_pe_1["close"].iloc[-1]
    print(f"Close price of 1-minute candle for PE: {close_price}")


def place_order(token, entry, stoploss, st_date_1min, end_date_1min):
    global orderPlaced
    ...
    if not orderPlaced:
        ...
        def check_order(token, entry, stoploss_dict, st_date_1min, end_date_1min, trailStopLoss, bookProfit):
            global orderPlaced
            candle_df = hist_data(token, "ONE_MINUTE", st_date_1min, end_date_1min, instrument_list)
            current_price = candle_df["close"].iloc[-1]
            print(f"Current price: {current_price}, token: {token}, entry: {entry}, stoploss: {stoploss_dict['value']}, st_date: {st_date_1min}, end_date: {end_date_1min}, trailStopLoss: {trailStopLoss}, bookProfit: {bookProfit}")
        ...
        schedule.every(1).minutes.do(
            functools.partial(check_order, token, entry, stoploss_dict, st_date_1min, end_date_1min, trailStopLoss, bookProfit)
        ).tag('order_check')
...
def execute_strategy():
    ...
    elif pe_bb == candle_df_pe_5min["high"] and pe_rsi > 60 and not orderPlaced:
